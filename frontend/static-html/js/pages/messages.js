...
export async function init() {
    ...
    try {
      ...
      conversation.message_set.reverse().forEach((message) => {
        ...
        messageElement.classList.add(
          message.sender === getUsername() ? "sent" : "received",
        );
  ...
  function setupWebSocket(conversationId) {
    ...
    socket = new WebSocket(
      `${WS_URL}/chat/${conversationId}?token=${getToken()}`,
    );
  ...
  async function inviteToMatch() {
    if (currentConversationId) {
      ...
      const otherUser = conversation.participants.find(
        (p) => p.username !== getUsername(),
      );
      if (otherUser) {
        send_to_online_sock("fight " + otherUser.username);
        console.log(
          `${getUsername()} ${i18n.t("messages.invite_to_match_message")}`,
        );
        socket.send(
          JSON.stringify({
