...
export function draw(state, scores) {
  ...
  //draw text
  {
    buff_ctx.fillText(
      scores[0] + " - " + scores[1],
      (VIEW_DIMENSIONS[0] / 2) | 0,
      (VIEW_DIMENSIONS[1] / 3) | 0,
    );
    if (state.countdown_frames > 0)
      buff_ctx.fillText(
        "" + (((state.countdown_frames + 59) / 60) | 0),
        (VIEW_DIMENSIONS[0] / 2) | 0,
        ((VIEW_DIMENSIONS[1] * 2) / 3) | 0,
      );
  }
  //draw ball
  {
...
export function render_rounds(rounds) {
  ...
  for (let i = 0; i < rounds.length; i++) {
    ...
    for (let j = 0; j < round.length; j++) {
      let content = round[j];
      if (content === null) content = "ø";
      buff_ctx.fillText(
        content,
        (VIEW_DIMENSIONS[0] * (i + 1)) / (rounds.length + 1),
        (VIEW_DIMENSIONS[1] * (j + 1)) / (round.length + 1),
      );
    }
  }
  ctx.drawImage(BUFF, 0, 0);
